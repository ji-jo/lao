 ...
 * backgrounds render as a live DOM layer beneath the canvas; other
 * background kinds are painted into the canvas itself.
 */
export function LaoComposition({ project }: LaoCompositionProps) {
  ...
  useEffect(() => {
    ...
    function paint() {
      ...
      if (!isShader) paintBackground(ctx, project, { onImageReady: paint });
      paintProjectFrame(ctx, project, frame, { clear: false });
    }
    paint();
  }, [project, frame, isShader]);
